...
echo "typescript $(echo_if $(program_is_installed typescript))"
...
function q_to_install {
  printf "Do you want to install $(highlight $2) globally? (y/n) "
  read res
...
read res
if [ "$res" == "yes" ] || [ "$res" == "y" ] ; then
  for var in "node" "gulp" "webpack" "eslint" "typescript" "brew" "gem"
  do
    if [ $(program_is_installed $var) == 0 ] ; then
      q_to_install $(program_is_installed $var) $var
    fi
  done
else
